...
const tokenrequest = () => {
  const [data, setData] = useState([]);
    ...
    useEffect(() => {
        axios.get('http://localhost:3000/tokenusers')
        .then(res => {
            setData(res.data)
        })
      ...
      const handleDelete = (id,name) => {
        if(window.confirm(`Are you sure want to delete ${name}`)){
            axios.delete('http://localhost:3000/tokenusers/'+ id)
    ...
    const handleAllow = (name,email,id,tpackage,amount) =>{
    console.log("Mubasshir",name,email,id,tpackage,amount,comment);
        ...
        try {
          ...
          axios
            .post('http://localhost:3000/tokendetails',{name,email,id,tpackage,amount,comment})
            .then((response) => {
                            ...
                            <tr>
                                ...
                                <button
                                    className='w-full bg-gray-800 hover:bg-green-900 text-white text-sm py-2 px-4 font-semibold rounded focus:outline-none focus:shadow-outline h-10'
                                     type='button'
                                     onClick={() => handleAllow(Item.name,Item.email,Item.ID,Item.selectedPackage,Item.amount)}>
                                     Approve
                                 </button>
