...
const Complains = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:3000/complains')
        .then(res => {
            setData(res.data)
        })
       ...
       const Filter = (event) =>{
        setData(data.filter(f => f.Name.toLowerCase().includes(event.target.value.toLowerCase())))
      }
      const handleDelete = (id,name) => {
        if(window.confirm(`Are you sure want to delete ${name}`)){
            axios.delete('http://localhost:3000/complains/'+ id)
                            ...
                            <td>
                             ...
                             <button
                                className='w-full bg-gray-800 hover:bg-red-900 text-white text-sm py-2 px-5 font-semibold rounded focus:outline-none focus:shadow-outline h-10'
                                 type='button' onClick={() => handleDelete(Item._id,Item.Name)}>
                                 Clear
                             </button> 
                            </td>
                        </tr>
